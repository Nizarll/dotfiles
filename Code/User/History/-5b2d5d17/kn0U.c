...
void bin_t_insert(struct Node* root, char* key, struct Route* route) {
    // If the root is NULL, create a new node and return it as the new root
    if (root == NULL) {
        return bin_t_create_node(key, route);
    }

    // Otherwise, proceed with the regular insertion procedure
    ...
    if (cmp < 0) {
        ...
        // Key already exists, handle if needed
...
// Function to lookup a node in the binary tree
...
void bin_t_free(struct Node* root) {
    if (root != NULL) {
        ...
        free(root->key);
        // Free other allocated resources in the Route structure if any
        free(root);
    }
}